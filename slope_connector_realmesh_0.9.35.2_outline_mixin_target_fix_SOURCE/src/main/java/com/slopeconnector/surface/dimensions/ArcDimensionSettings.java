 ...
 *
 * <p>Compatibility note: the embedded field/method names cannot safely be renamed.  In the UI the
 * original {@code width} field is now called "上下厚度", while the additional value historically
 * named {@code upDown} is now called "侧面宽度" because that matches its actual visual effect.  The
 * legacy Java/command names remain only so old settings and scripts keep working.</p>
 */
